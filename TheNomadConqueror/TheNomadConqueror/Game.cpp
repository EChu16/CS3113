	...
	if (state == IN_GAME) {
		...
		if (selectedLevel == FOREST || selectedLevel == CANDYLAND || selectedLevel == SNOW_TUNDRA) {
			...
			if (keys[SDL_SCANCODE_LEFT]) {
				...
				if (player->isOnSurface() && player->isWalking()) {
					if (player->getWalkingTime() >= 0.775f) {
						player->resetWalkingTime();
					}
					else if (player->getWalkingTime() >= 0.62f) {
						player->setCurrentSprite(pWalk2, 1.0f);
					}
					else if (player->getWalkingTime() >= 0.465f) {
						player->setCurrentSprite(pWalk3, 1.0f);
					}
					else if (player->getWalkingTime() >= 0.31f) {
						player->setCurrentSprite(pWalk2, 1.0f);
					}
					else if (player->getWalkingTime() >= 0.155f) {
						player->setCurrentSprite(pWalk1, 1.0f);
					}
				}
			...
			//Move Right
				...
				if (player->isOnSurface()) {
					player->setCurrentSprite(pStandingSprite, 0.0f);
				}
				if (player->hasTinyGun()) { tiny_ray_gun->changeXVel(0.0f);	}
			}
			if (player->hasTinyGun()) {
			...
			if (keys[SDL_SCANCODE_SPACE]) {
				if (player->isOnSurface() || player->canDoubleJump()) {
					...
					player->setCurrentSprite(pJumpSprite, 1.0);
...
void Game::Update(float elapsed) {
	...
	if (state == IN_GAME) {
		...
		//Items
		for (int i = 0; i < coins.size(); i++) {
			if (player->collidesWith(coins[i]) && coins[i]->isAlive()) {
...
void Game::LoadAllTexturesandSound() {
	...
	pJumpSprite = SheetSprite(playerSprites, 438.0f / 508.0f, 67.0f / 208.0f, 72.0f / 508.0f, 69.0f / 208.0f, 2.0f);
	...
	pWalk1 = SheetSprite(playerSprites, 0.0f / 508.0f, 72.0f / 208.0f, 72.0f / 508.0f, 69.0f / 208.0f, 2.0f);
	pWalk2 = SheetSprite(playerSprites, 73.0f / 508.0f, 72.0f / 208.0f, 72.0f / 508.0f, 69.0f / 208.0f, 2.0f);
	pWalk3 = SheetSprite(playerSprites, 146.0f / 508.0f, 72.0f / 208.0f, 72.0f / 508.0f, 69.0f / 208.0f, 2.0f);
