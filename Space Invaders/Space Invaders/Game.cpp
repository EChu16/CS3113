...
void Game::GameOver() {
	...
	program->setModelMatrix(gameMatrix);
	std::string spaceText = "Game Over";
	DrawText(fontSprites, spaceText, 0.7f, -0.35f);

	if (victory == true) {
		gameMatrix.identity();
		gameMatrix.Translate(-0.9f, 0.35f, 0.0f);
		program->setModelMatrix(gameMatrix);
		std::string invadersText = "You Win";
		DrawText(fontSprites, invadersText, 0.7f, -0.4f);
	}
	else {
		gameMatrix.identity();
		...
		program->setModelMatrix(gameMatrix);
		std::string invadersText = "You Lose";
		DrawText(fontSprites, invadersText, 0.7f, -0.4f);
	}
	...
	gameMatrix.Translate(-1.5f, -1.0f, 0.0f);
	program->setModelMatrix(gameMatrix);
	std::string instructionsText = "Press Enter to close";
	DrawText(fontSprites, instructionsText, 0.48f, -0.3);
}
		...
		if (state == GAMEPLAY){
			if (event.type == SDL_KEYDOWN) {
				...
				//Shoot bullet
					...
					player->shootBullet(bullets, 1.0f);
				}
			}
		}
		if (state == GAMEOVER) {
...
bool Game::didGameEnd() const { victory == true; }
